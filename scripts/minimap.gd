...
func _draw():
	...
	if m_key_held:
		var _zoom_text = "ZOOM: " + str(round(1.0 / scale_factor))
	...
	draw_circle(center, 4, Color(2, 2, 0))      # Super bright yellow outer
	draw_circle(center, 3, Color(2, 2, 2))      # Super bright white middle
	draw_circle(center, 2, Color(0, 2, 0))      # Super bright green center
	...
	if player:
		...
		# Draw asteroids relative to player - small white circles
		for asteroid in asteroids:
			if is_instance_valid(asteroid) and asteroid.is_inside_tree():
				...
				# Only draw if on screen
				if screen_pos.x >= 0 and screen_pos.x <= size.x and screen_pos.y >= 0 and screen_pos.y <= size.y:
					draw_circle(screen_pos, 2, Color.WHITE)     # Small white circles for asteroids
