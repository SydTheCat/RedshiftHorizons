...
func _ready():
	...
	if not ray_cast or not core_mesh or not particles or not hit_particles:
		push_warning("Laser beam missing required child nodes - some features may be disabled")
		return
	...
	# Get material references safely
	if core_mesh.mesh and core_mesh.mesh.material:
		core_material = core_mesh.mesh.material
	else:
		push_warning("Core mesh or material not found - visual effects may be limited")
		
	if particles.process_material:
		particle_material = particles.process_material
	else:
		push_warning("Particle material not found - particle effects may be limited")
