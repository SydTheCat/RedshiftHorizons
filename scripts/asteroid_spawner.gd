...
func _on_spawn_timer_timeout():
	if enabled and asteroid_scene != null and _active_asteroids.size() < max_active_asteroids and is_inside_tree():
		spawn_asteroid()
...
func spawn_asteroid():
	...
	# Calculate movement direction towards player (optional)
	var direction_to_player = (player_pos - spawn_position).normalized()
	var speed = _rng.randf_range(speed_min, speed_max)  # Random speed
